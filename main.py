...
# threading es el único modo compatible con Python 3.11 + Railway sin deps extra
...
PLAYER_SPEED      = 250
PLAYER_RADIUS     = 20
PROJECTILE_RADIUS = 6   # la mitad de PROJECTILE_WIDTH
...
def generate_solid_obstacles():
    ...
    obs = []
    for _ in range(70):
        x = rng() * MAP_WIDTH;  y = rng() * MAP_HEIGHT
        w = 40 + rng() * 70;   h = 28 + rng() * 50
        rng()
...
def build_spatial_grid(obstacles, cell):
    ...
    for o in obstacles:
        ...
        cx0 = int((o["x"] - rad) / cell);  cx1 = int((o["x"] + rad) / cell)
        cy0 = int((o["y"] - rad) / cell);  cy1 = int((o["y"] + rad) / cell)
...
def nearby(x, y):
    cx = int(x / GRID_CELL);  cy = int(y / GRID_CELL)
    seen = set(); out = []
    for dcx in (-1,0,1):
        for dcy in (-1,0,1):
...
def circle_vs_rect(cx, cy, cr, rx, ry, rw, rh):
    nx = clamp(cx, rx-rw, rx+rw);  ny = clamp(cy, ry-rh, ry+rh)
    dx = cx-nx;  dy = cy-ny
...
def resolve_player(plr):
    ...
    for o in nearby(plr["x"], plr["y"]):
        ox, oy = o["x"], o["y"]
        if o["type"] == "rock":
            or_ = o["r"]; dx = plr["x"]-ox; dy = plr["y"]-oy
            d2 = dx*dx+dy*dy; md = or_+r
            if d2 < md*md:
                d = math.sqrt(d2) or 0.001
                plr["x"] = ox + dx/d*md;  plr["y"] = oy + dy/d*md
        else:
            ow, oh = o["rw"], o["rh"]
            if circle_vs_rect(plr["x"], plr["y"], r, ox, oy, ow, oh):
                dx = plr["x"]-ox;  dy = plr["y"]-oy
                ox_ = ow+r-abs(dx);  oy_ = oh+r-abs(dy)
                if ox_ < oy_: plr["x"] += ox_ * (1 if dx>=0 else -1)
                else:          plr["y"] += oy_ * (1 if dy>=0 else -1)
...
def proj_hits(px, py):
    ...
    for o in nearby(px, py):
        ...
        if o["type"] == "rock":
            dx=px-ox; dy=py-oy
            if dx*dx+dy*dy < (o["r"]+pr)**2: return True
        else:
            if circle_vs_rect(px, py, pr, ox, oy, o["rw"], o["rh"]): return True
    return False
...
# GAME LOOP
# Optimizaciones:
#  • 10 ticks/s — física simple, Railway free lo aguanta bien
#  • Emitir JSON pre-serializado con orjson si está disponible,
#    sino json estándar
#  • Evitar crear dicts intermedios — tuplas directas
#  • logger=False elimina el overhead de logs de socketio
# ──────────────────────────────────────────────────────────
def game_loop():
    global last_update
    ...
    while True:
        ...
        for pid, pr in list(projectiles.items()):
            ...
            px, py = pr["x"], pr["y"]
            if (pr["lifetime"] <= 0
                    or px < 0 or px > MAP_WIDTH
                    or py < 0 or py > MAP_HEIGHT
                    or proj_hits(px, py)):
                dead.append(pid); continue

            ot = pr["owner_team"]; oi = pr["owner_id"]; hit = False
            for uid, plr in players.items():
                if uid==oi or plr["team"]==ot or plr["dead"]: continue
                dx=plr["x"]-px; dy=plr["y"]-py
                if dx*dx+dy*dy < HIT_DIST_SQ:
                    plr["hp"] -= pr["damage"]; dead.append(pid); hit=True
                    if plr["hp"] <= 0:
                        plr["hp"]=0; plr["dead"]=True; plr["dead_timer"]=3.0
                        socketio.emit("player_died",{
                            "id":uid,"victim_team":plr["team"],
                            "killer_id":oi,
                            "killer_team":players.get(oi,{}).get("team","")})
                    break
            if hit: continue

        for pid in set(dead): projectiles.pop(pid, None)
        ...
        for uid, plr in list(players.items()):
            if plr["dead"]:
                ...
                if plr["dead_timer"] <= 0:
                    ...
                    plr["x"]=clamp(sp["x"],0,MAP_WIDTH)
                    plr["y"]=clamp(sp["y"],0,MAP_HEIGHT)
                    plr["hp"]=100; plr["dead"]=False; plr["dead_timer"]=0
                    socketio.emit("player_respawned",{"id":uid})

        # Estado compacto — int coords (1px precision es suficiente a 10 ticks)
        state = {
            "p": {uid:[int(p["x"]),int(p["y"]),p["hp"],p["team"],p["shape"],
                       int(p["dead"]),round(p.get("angle",0),2)]
                  for uid,p in players.items()},
            "b": {bid:[int(b["x"]),int(b["y"]),round(b["dx"],2),round(b["dy"],2),b["owner_team"]]
                  for bid,b in projectiles.items()}
        }
        socketio.emit("game_state", state)

        elapsed = time.time() - t0
        socketio.sleep(max(interval - elapsed, 0))
...
@app.route("/game.js")
def js():    return send_from_directory(".", "game.js")


@socketio.on("connect")
def on_connect(): pass   # sin print — reduce I/O
...
def on_join(data):
    ...
    if team not in ("red","blue"): team = "red"
    ...
    emit("joined",{"id":sid,"x":players[sid]["x"],"y":players[sid]["y"],
...
def on_shoot(data):
    ...
    if plr["dead"]: return
    now = time.time()
    if now - plr["last_shot"] < SHAPE_FIRE_RATE[plr["shape"]]: return
    plr["last_shot"] = now
