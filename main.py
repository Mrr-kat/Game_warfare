...
# 20 ticks para mejor respuesta en movimiento
TICK_RATE = 20
MAX_PROJECTILES = 50
...
def generate_solid_obstacles():
    ...
    # Bushes (solo decorativos, no colisión)
    for _ in range(60):
...
# ──────────────────────────────────────────────────────────
# GAME LOOP OPTIMIZADO CON SECUENCIAS
# ──────────────────────────────────────────────────────────
def game_loop():
    global last_update
    ...
    while True:
        ...
        # Construir estado con secuencia
        state = {
            "p": {uid: [
                int(p["x"]), int(p["y"]), 
                p["hp"], p["team"], p["shape"],
                1 if p["dead"] else 0, 
                round(p.get("angle", 0), 2),
                p.get("last_seq", 0)  # Secuencia del último input procesado
...
def on_join(data):
    ...
    players[sid] = {
        ...
        "angle": 0, 
        "last_shot": 0,
        "last_seq": 0  # Última secuencia de input procesada
    }
    emit("joined", {
        "id": sid, 
        "x": players[sid]["x"], 
...
def on_input(data):
    ...
    # Obtener secuencia del input
    seq = data.get("seq", 0)
    keys = data.get("keys", {})
    dt = min(data.get("dt", 0.033), 0.05)
    plr["angle"] = data.get("angle", 0)
    plr["last_seq"] = seq  # Guardar la última secuencia procesada
